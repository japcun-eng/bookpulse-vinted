...
let data;let showingTop=false;function render(items){const query=document.querySelector('#search').value.trim().toLowerCase();const filtered=query?items.filter(x=>`${x.title} ${x.author||''} ${x.isbn||''}`.toLowerCase().includes(query)):items;const visible=query?filtered.slice(0,200):filtered.slice(0,100);document.querySelector('#rows').innerHTML=visible.map((x,i)=>`<tr><td><b>${i+1}. ${x.title}</b><br><span class='muted'>${x.author||'Nieznany autor'} · ${x.isbn||'brak ISBN'}</span></td><td>${x.occurrences}</td><td>${formatDate(x.first_seen)}</td><td>${x.last_seen_is_disappearance?formatDate(x.last_seen):'<span class="muted">nadal aktywna</span>'}</td><td>${x.sell_through_24h}%</td><td>${x.median_price.toFixed(2)} zł</td><td>${x.recommended_buy.toFixed(2)} zł</td><td class='pill'>${x.expected_profit.toFixed(2)} zł</td><td>${x.confidence}%</td></tr>`).join('')||`<tr><td colspan='9' class='empty'>Brak wyników wyszukiwania</td></tr>`}function setTab(isTop){showingTop=isTop;document.querySelector('#rankingTab').classList.toggle('active',!showingTop);document.querySelector('#topTab').classList.toggle('active',showingTop);render(showingTop?data.top50:data.items)}document.querySelector('#rankingTab').onclick=()=>setTab(false);document.querySelector('#topTab').onclick=()=>setTab(true);document.querySelector('#search').oninput=()=>render(showingTop?data.top50:data.items);fetch('/api/summary').then(r=>r.json()).then(d=>{data=d;document.querySelector('#offers').textContent=d.offers;document.querySelector('#titles').textContent=d.titles;document.querySelector('#fast').textContent=d.fast_sales;render(d.items)})
