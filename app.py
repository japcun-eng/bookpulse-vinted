...
let data;function render(items){document.querySelector('#rows').innerHTML=items.map((x,i)=>`<tr><td><b>${i+1}. ${x.title}</b><br><span class='muted'>${x.author||'Nieznany autor'} · ${x.isbn||'brak ISBN'}</span></td><td>${x.occurrences}</td><td>${formatDate(x.first_seen)}</td><td>${x.last_seen_is_disappearance?formatDate(x.last_seen):'<span class="muted">nadal aktywna</span>'}</td><td>${x.sell_through_24h}%</td><td>${x.median_price.toFixed(2)} zł</td><td>${x.recommended_buy.toFixed(2)} zł</td><td class='pill'>${x.expected_profit.toFixed(2)} zł</td><td>${x.confidence}%</td></tr>`).join('')||`<tr><td colspan='9' class='empty'>Brak danych</td></tr>`}function setTab(top){document.querySelector('#rankingTab').classList.toggle('active',!top);document.querySelector('#topTab').classList.toggle('active',top);render(top?data.top50:data.items)}document.querySelector('#rankingTab').onclick=()=>setTab(false);document.querySelector('#topTab').onclick=()=>setTab(true);fetch('/api/summary').then(r=>r.json()).then(d=>{data=d;document.querySelector('#offers').textContent=d.offers;document.querySelector('#titles').textContent=d.titles;document.querySelector('#fast').textContent=d.fast_sales;render(d.items)})
